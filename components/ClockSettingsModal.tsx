...
import { ClockConfig, TimeFormat, DateFormat } from '../types';
...
const ClockSettingsModal: React.FC<ClockSettingsModalProps> = ({ isOpen, onClose, config, onSave }) => {
  ...
  // Reset temp config when opening
  React.useEffect(() => {
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                onClick={() => setTempConfig({ ...tempConfig, timeFormat: '24h' })}
                className={`p-3 rounded-xl border text-center transition-all ${
                  tempConfig.timeFormat === '24h' 
              ...
              <button
                onClick={() => setTempConfig({ ...tempConfig, timeFormat: '12h' })}
                className={`p-3 rounded-xl border text-center transition-all ${
                  tempConfig.timeFormat === '12h' 
                    ? 'bg-blue-500/20 border-blue-500 text-white' 
                    : 'bg-white/5 border-transparent hover:bg-white/10 text-white/70'
                }`}
              >
            ...
            <div className="mt-4">
               <label className="flex items-center gap-3 cursor-pointer group p-2 rounded-lg hover:bg-white/5 transition-colors">
                 <div className={`w-10 h-5 rounded-full p-0.5 transition-colors ${tempConfig.showSeconds ? 'bg-blue-500' : 'bg-white/20'}`}>
          ...
          <div>
            ...
            <div className="space-y-2">
              {[
                ...
                <button
                  key={opt.id}
                  onClick={() => setTempConfig({ ...tempConfig, dateFormat: opt.id as DateFormat })}
                  className={`w-full flex items-center justify-between p-3 rounded-lg border transition-all ${
                    tempConfig.dateFormat === opt.id
