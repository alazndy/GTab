...
export const CategoryFilterWidget: React.FC = () => {
  ...
  const { activeCategories, uniqueProfiles } = useMemo(() => {
    // ⚡ Bolt Optimization: Consolidate multiple O(N) array traversals (.map, .flatMap)
    // into a single loop to minimize intermediate object allocations and redundant iterations
    // when deriving unique UI filter lists from the shortcuts array.
