...
## 2023-10-27 - Consolidating derived state computations
**Learning:** When deriving multiple unique sets of data from a single large source array (e.g., categories and profiles from a shortcuts list), chaining array methods like `map` and `flatMap` causes unnecessary allocations and multiple loops over the same data.
**Action:** Consolidate these calculations into a single `useMemo` hook using a single loop (`for...of` or standard `for` loop). This pattern minimizes iterations and avoids intermediate object allocations, improving performance significantly for large arrays.
