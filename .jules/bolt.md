...
## 2025-05-29 - Combine derived state array iterations
**Learning:** When deriving multiple unique sets of data from a single large source array (like extracting both unique categories and unique profiles from a list of shortcuts), using separate `useMemo` hooks with declarative methods like `.map()` and `.flatMap()` causes the array to be iterated over multiple times, which allocates intermediate arrays and harms performance on large datasets.
**Action:** Consolidate the calculations into a single `useMemo` hook using a single `for...of` loop. This avoids intermediate array allocations and minimizes iterations, significantly improving performance.
