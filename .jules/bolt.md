...
## 2023-10-28 - Consolidate array derivations in useMemo
**Learning:** When deriving multiple unique sets of data from a single large source array (e.g., categories and profiles from a shortcuts list), using chained declarative methods (`map`, `flatMap`, `Set`) across multiple `useMemo` hooks is inefficient as it iterates the array multiple times and creates many intermediate arrays.
**Action:** Consolidate derivations into a single `useMemo` hook using a standard `for` loop to minimize iterations and avoid intermediate object allocations.
