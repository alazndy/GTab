...
## 2025-03-09 - Consolidate Multiple Map/Filter/Set Chains into Single Loops
**Learning:** Chaining array methods like `.map()`, `.flatMap()`, and converting multiple times to `Set` introduces severe performance penalties for operations that derive state from large data arrays. This creates intermediate objects, resulting in excess CPU and memory usage during renders.
**Action:** When computing derived state from arrays for distinct pieces of data, prefer consolidating iterations into a single `useMemo` object that uses a standard `for` loop. This traverses the array once, processes required fields simultaneously, and optimizes calculations, significantly decreasing overall computation times.
