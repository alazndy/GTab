...
## 2025-03-09 - Consolidate declarative map/flatMap operations into a single loop
**Learning:** When deriving multiple sets of data (like categories and profiles) from a single large array using declarative methods (`map`, `flatMap`, `Set`), React applications suffer from performance degradation due to multiple full-array iterations and intermediate object allocations.
**Action:** Consolidate data derivation into a single `useMemo` block using standard `for` loops to process multiple datasets in a single pass, which drastically reduces execution time and memory allocations.
