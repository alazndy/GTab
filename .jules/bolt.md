...
## 2025-03-09 - Consolidate chained array methods into single loop
**Learning:** Chaining array methods like `.map()`, `.flatMap()`, and converting to `Set` introduces multiple iterations and intermediate object allocations. When deriving multiple unique sets of data from a large source array (e.g., categories and profiles from a shortcuts list), these chained methods can become performance bottlenecks, taking significantly more time than a single manual iteration.
**Action:** Consolidate the calculation of multiple derived state variables into a single `useMemo` block that uses a single `for...of` or `for` loop to iterate through the source array exactly once.
