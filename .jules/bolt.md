...
## 2025-03-09 - Consolidate array iterations in useMemo
**Learning:** When deriving multiple unique sets of data from a single large source array (e.g., categories and profiles from a shortcuts list), using separate `useMemo` hooks with chained declarative methods (`map`, `flatMap`, `Set`) results in multiple iterations over the array and intermediate object allocations.
**Action:** Consolidate the calculations into a single `useMemo` hook using a single standard `for` loop to populate Sets. This pattern is significantly more efficient as it minimizes iterations and avoids intermediate array allocations.
