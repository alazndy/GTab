...
## 2025-05-28 - Consolidate multiple data derivation loops into a single pass
**Learning:** When deriving multiple unique sets of data from a single large source array (e.g., categories and profiles from a shortcuts list), chaining declarative methods like `map`, `flatMap`, and `Set` in multiple `useMemo` hooks introduces significant performance overhead due to multiple iterations over the same data and excessive intermediate array allocations.
**Action:** Consolidate the calculations into a single `useMemo` hook using a single `for...of` or standard `for` loop. This pattern is significantly more efficient as it minimizes iterations and avoids intermediate object allocations.
