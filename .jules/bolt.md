...
## 2025-03-09 - Consolidate array derivations in React components
**Learning:** Deriving multiple unique sets (like categories and profiles) from a single large source array using separate chained declarative methods (`.map`, `.flatMap`, `...new Set`) causes multiple expensive iterations over the same data.
**Action:** Consolidate these derivations into a single `useMemo` block that iterates through the source array exactly once using a standard `for` loop, populating all necessary sets simultaneously. This minimizes intermediate object allocations and massively improves execution speed.
