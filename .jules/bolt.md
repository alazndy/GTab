...
## 2025-03-10 - Consolidate loops for derived state in React components
**Learning:** When deriving multiple unique sets of data from a single large source array (e.g., categories and profiles from a shortcuts list), using separate `useMemo` hooks with chained declarative methods (`map`, `flatMap`, `Set`) causes redundant iterations and unnecessary intermediate object allocations. In `CategoryFilterWidget`, this bottleneck was impacting performance on every shortcuts update.
**Action:** Consolidate these calculations into a single `useMemo` hook using a single `for...of` loop. This approach significantly minimizes iterations and memory overhead, resulting in faster and more efficient state derivation.
