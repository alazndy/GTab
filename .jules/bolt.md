...
## 2025-03-09 - Consolidating Iterative Derivations
**Learning:** When deriving multiple unique sets of data from a single large source array (e.g., categories and profiles from a shortcuts list), using chained declarative methods (`map`, `flatMap`) across multiple `useMemo` hooks introduces significant performance overhead by iterating through the array multiple times and allocating intermediate objects.
**Action:** Consolidate calculations into a single `useMemo` hook using a standard `for` loop to iterate through the array once, minimizing allocations and improving performance.
